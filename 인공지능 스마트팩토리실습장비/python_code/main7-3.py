...
# 엑셀 파일 열기 또는 생성
try:
    workbook = openpyxl.load_workbook('QR코드_데이터저장.xlsx')
except FileNotFoundError:
    workbook = openpyxl.Workbook()
    workbook.save('QR코드_데이터저장.xlsx')  # 새로운 엑셀 파일 생성
...
while True:
    ...
    if qr_save and qr_data:
        #시간과 qrcode에서 읽은 값을 엑셀에 저장
        now = datetime.now()
        current_time = now.strftime("%Y-%m-%d %H:%M:%S")  
        sheet.cell(row=row, column=1).value = current_time
        sheet.cell(row=row, column=2).value = qr_data
        row += 1
        workbook.save('QR코드_데이터저장.xlsx') 
        qr_save = False
